%% Solves SOE model with a borrowing constraint using projection
...
% convergence
error = muc( exp( z_mesh ) + r * b_mesh - b_p ) - muc_p + 2 * delta * b_p;
ssr = sum( sum( error.^2 ) );
...
%% Risky steady state
disp('Find stationary point')
res = @(b) b-interp2( z_mesh,b_mesh,b_p,0,b,'spline' ); 
b0 = 0; 
b_bar = fzero(res,b0,optimset('Display','iter'));
c_bar = ( 1 + (r-1)*b_bar ) / (1 + chi);
h_bar = 1 - chi * c_bar;

disp(['Risky steady state: b/c = ',num2str(100*b_bar/c_bar),'%'])
...
projection.sim.c = cons;  % as percent of average consumption
projection.sim.b = bonds;  % relative to average consumption
projection.sim.h = hours; % relative to average hours
save('results/projection_sim.mat','projection')
...
projection.irf.c.epsz = mean(cons_mat(:,irf_drop+1:end))/ mean(cons);  % as percent of average consumption
projection.irf.b.epsz = mean(bonds_mat(:,irf_drop+1:end))/mean(cons);  % relative to average consumption
projection.irf.h.epsz = mean(hours_mat(:,irf_drop+1:end))/ mean(hours); % relative to average hours
projection.irf.b_limit.epsz = b_limit./mean(cons);
